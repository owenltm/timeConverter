...
function TimeInput({index, timeValue, zoneValue, handleTimeChange, handleZoneChange, handleAddTime, handleRemoveTime}) {
  ...
  const onAutocompleteClick = (id) => {
    setTimezone(autocompletes[id]);

    handleZoneChange(index, autocompletes[id], timeValue);
  ...
  const calculateSky = (slider) => {
    ...
    const baseRGB = [14, 165, 233];;
    const rgbDiff = [2, 91, 123];
  ...
  return (
    <div className='time-input flex-1 relative'>
      ...
      <div className='flex flex-col h-full transition-colors duration-300' style={{ backgroundColor: 'rgb(' + skyRGB[0] + ',' + skyRGB[1]+ ',' + skyRGB[2] + ')' }}>
        <div className='flex flex-col items-center py-4'>
          ...
          {autocompletes.length > 0 && <div className='autocomplete-list mt-1 w-2/5 max-h-60 rounded overflow-y-scroll overflow-x-hidden text-center absolute top-16'>
            {autocompletes.map((item, i) => <div className={'autocomplete-item py-2 hover:bg-sky-200 ' + (i === currentFocus ? "bg-sky-200" : "bg-sky-100")} onClick={() => onAutocompleteClick(i)}>
              <p>{item}</p>
            </div>)}
          </div>}
